...
public class Player extends Entity {
	...
	public void tick() {
		...
		/*
		 * if (mouseShoot) { // atirar com mouse mouseShoot = false; if (haveGun == true
		 * && ammo > 0) { ammo--; int px, py = 8; double angle = 0;
		 * 
		 * if (dir == right_dir || dir == down_dir) { px = 16;
		 * 
		 * angle = Math.atan2(my - (this.getY() + py - Camera.y), mx - (this.getX() + px
		 * - Camera.x));
		 * 
		 * } else { px = -3; angle = Math.atan2(my - (this.getY() + py - Camera.y), mx -
		 * (this.getX() + px - Camera.x));
		 * 
		 * } double dx = Math.cos(angle); double dy = Math.sin(angle); Shoot shoot = new
		 * Shoot(this.getX() + px, this.getY() + py, 4, 4, null, dx, dy);
		 * Game.bullets.add(shoot); } }
		 */
